...
extension APDUResponse {
    ...
    func decryptDES(sequence: [UInt8], signatureKey: [UInt8], cryptoKey: [UInt8], iv: [UInt8]) throws -> APDUResponse {
...
extension APDUResponse {
    /**7.1.8 Commands and Responses under SM - Responses*/
    func decryptAES(sequence: [UInt8], signatureKey: [UInt8], cryptoKey: [UInt8], iv: [UInt8]) throws -> APDUResponse {
        guard let tlvRecords = TKBERTLVRecord.sequenceOfRecords(from: Data(self.data)) else {
            throw NfcDigitalIdError.errorDecodingAsn1
        }
        ...
        if let data = data {
            ...
            clearData = Utils.unpad(try AES.decrypt(key: cryptoKey, message: data, iv: iv))
